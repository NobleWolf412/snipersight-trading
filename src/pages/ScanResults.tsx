...
import { Target, WarningCircle, CaretLeft, CaretRight, ArrowCounterClockwise, SquaresFour, List as ListIcon } from '@phosphor-icons/react';
import { ScanResultsTable } from '@/components/ScanResults/ScanResultsTable';
    ...
    <div className="min-h-screen bg-[#030504] text-zinc-100 font-sans selection:bg-[#00ff88]/30 relative">
      ...
      <main className="relative z-10 min-h-screen flex flex-col items-center">
        ...
        <div className="w-full max-w-7xl px-8 pb-24">
          ...
          {/* Toggle Controls */}
          <div className="flex items-center justify-between mb-8 border-b border-zinc-800/60">
            ...
            {/* View Mode Switcher */}
            {activeTab === 'targets' && results.length > 0 && (
              <div className="flex items-center gap-1 p-1 bg-zinc-900/60 rounded-lg border border-zinc-800 mb-4">
                <button
                  onClick={() => setViewMode('cards')}
                  className={cn(
                    "p-1.5 rounded-md transition-all",
                    viewMode === 'cards' ? "bg-zinc-800 text-[#00ff88]" : "text-zinc-500 hover:text-zinc-300"
                  )}
                  title="Card View"
                >
                ...
                <button
                  ...
                  className={cn(
                    "p-1.5 rounded-md transition-all",
                    viewMode === 'table' ? "bg-zinc-800 text-[#00ff88]" : "text-zinc-500 hover:text-zinc-300"
                  )}
                  title="Table View (with Rationale)"
                >
