    ...
    try {
      ...
      while ((jobStatus === 'queued' || jobStatus === 'running') && pollAttempts < MAX_POLL_ATTEMPTS) {
        ...
        if (jobStatus === 'completed') {
          ...
          // DEBUG: Log raw rejection data from API
          console.log('[ScannerSetup] Raw rejections from API:', JSON.stringify(rawRejections, null, 2));
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            {recommendation?.matrix && (
              <div className="max-w-4xl mx-auto w-full -mt-4 mb-8 grid grid-cols-1 md:grid-cols-3 gap-3 relative z-20 px-2 lg:px-0">
                {['1w', '1d', '4h'].map(tf => {
                  const data = (recommendation.matrix as any)[tf];
                  if (!data) return null;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <section className="glass-card glow-border-green p-5 lg:p-6 rounded-2xl h-full flex flex-col relative overflow-hidden group">
                ...
                <div className="flex-1 flex flex-col justify-between gap-10 lg:gap-12 relative z-10">
                  ...
                  {/* 0. Targeted Recon (Single Symbol) */}
                  ...
                  {scanConfig.targetSymbol && (
                    <div className="absolute inset-0 flex items-center justify-center z-20">
                      <div className="bg-black/80 border border-[#00ff88]/30 px-6 py-3 rounded-full backdrop-blur-md shadow-[0_0_30px_rgba(0,255,136,0.1)] flex flex-col items-center gap-1">
                        ...
                        <span className="text-zinc-500 font-mono text-[10px] tracking-widest uppercase">Target Lock Active: {scanConfig.targetSymbol}</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            >
              {isScanning ? (
                ...
                <>
                  ...
                  {scanConfig.targetSymbol ? 'LOCK TARGET & SCAN' : 'ARM SCANNER'}
                </>
              )}
            </Button>
