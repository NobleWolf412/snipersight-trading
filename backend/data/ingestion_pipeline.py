...
class IngestionPipeline:
    ...
    def __init__(self, adapter, use_cache: bool = True):
        ...
        logger.info(
            f"Ingestion pipeline initialized with {adapter.__class__.__name__} (cache: {cache_status})"
        )
    ...
    def get_limit_for_mode(timeframe: str, mode_profile: str = "balanced") -> int:
        ...
        is_swing_mode = profile_lower in (
            "macro_surveillance",
            "overwatch",
            "stealth_balanced",
            "stealth",
    ...
    def fetch_multi_timeframe(
        ...
        if total_requests > 0:
            ...
            logger.debug(
                f"{symbol}: cache {cache_hits}/{total_requests} ({hit_rate:.0f}% hit rate)"
            )
    ...
    def parallel_fetch(
        ...
        logger.info(
            f"Parallel fetch complete: {len(results)} succeeded, " f"{len(failed_symbols)} failed"
        )
    ...
    def _to_pandas_freq(self, timeframe: str) -> Optional[str]:
        ...
        if timeframe.endswith("m"):
            return f"{timeframe[:-1]}min"  # m -> min
    ...
    def _fill_time_gaps(self, df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            # Use timestamp column as index, dropping it from columns to avoid ambiguity
        ...
        if len(df_filled) == len(df):
            # Restore timestamp as a column, reset index cleanly
        ...
        # Restore timestamp as a regular column from the DatetimeIndex
        df_filled["timestamp"] = df_filled.index

        return df_filled.reset_index(drop=True)

    def normalize_and_validate(self, df: pd.DataFrame, symbol: str, timeframe: str) -> pd.DataFrame:
        """
        ...
        if missing_cols:
            raise ValueError(f"Missing required columns for {symbol} {timeframe}: {missing_cols}")
        ...
        if not df.columns.is_unique:
            logger.warning(
                f"Found duplicate columns in {symbol} {timeframe}, keeping first: {df.columns[df.columns.duplicated()].tolist()}"
            )
        ...
        # Rename the index to avoid the ambiguity issue when both index.name and
        # a column share the same label
        df.index.name = None
    ...
    def fetch_universe_symbols(
        self, universe: str, quote_currency: str = "USDT", market_type: Optional[str] = None
    ) -> List[str]:
        """
        ...
        """
        universe_map = {"top10": 10, "top20": 20, "top50": 50, "top100": 100}
        ...
        if hasattr(self.adapter, "get_top_symbols"):
            ...
            try:
                return self.adapter.get_top_symbols(
                    n=n, quote_currency=quote_currency, market_type=market_type
                )
            except TypeError:
                # Fallback for adapters not yet updated
                logger.warning(
                    f"Adapter {self.adapter.__class__.__name__} does not support market_type in get_top_symbols"
                )
