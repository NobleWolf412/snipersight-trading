...
def scan(
    profile: str = typer.Option(
        "balanced", help="Strategy profile (balanced/trend/range/aggressive)"
    ),
    symbols: str = typer.Option(
        "top20", help="Symbol universe (top10/top20/top50 or comma-separated list)"
    ),
    ...
    try:
        ...
        # Parse symbol list
        if symbols == "top20":
            symbol_list = ["BTC/USDT", "ETH/USDT", "BNB/USDT", "SOL/USDT", "MATIC/USDT"][
                :5
            ]  # Limited for demo
        elif symbols == "top10":
        ...
        if results:
            ...
            for i, plan in enumerate(results, 1):
                ...
                typer.echo(
                    f"   Entry: ${plan.entry_zone.near_entry:.2f} - ${plan.entry_zone.far_entry:.2f}"
                )
...
@app.command()
def audit(verbose: bool = typer.Option(False, "--verbose", "-v", help="Show detailed output")):
    """
