...
| T13 | **ATR-regime classification tests FAILING on HEAD (§10 standing-fix surface)** — test_planner_atr_altstop::test_atr_regime_classification[0.8→normal, 1.5→elevated, 3.0→explosive] all FAIL | 2 | Found 2026-06-06 (pre-existing, NOT from T11; diff was comment-only). Touches the percentage-based ATR regime bands (§10 standing fix). On clean HEAD, so committed-code failure, not WIP. | Investigate: is the ATR%→band classifier actually wrong (standing-fix REGRESSION, escalate) or are the test's expected bands stale/the test drifted? Run the test, compare classifier output vs expected bands. |
