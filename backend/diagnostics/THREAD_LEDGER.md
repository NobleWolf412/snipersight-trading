...
| T11 | **Long-only WCL/DCL Gate-2 cycle bypass — possible bull/bear asymmetry (§10)** — scorer.py:224-232: `opposing_long and _in_wcl/_in_dcl` rescues a LONG into a strong-down regime via a cycle LOW; no `opposing_short and _in_wch/_in_dch` mirror for a SHORT into strong-up. | 3 | symmetry-guard SUSPECT (2026-06-06): production STEALTH counter-trend path (CHoCH L254-293) IS symmetric, so NOT a runtime break — but the cycle-zone rescue is long-only, echoing the historical LONG-bias class. | Confirm whether cycle_context exposes in_wch/in_dch (cycle-HIGH zones). If yes → add the SHORT mirror (symmetry-guard + §16). If no → add a one-line direction-agnostic rationale at scorer.py:226 citing the cycle_context contract limitation. |
