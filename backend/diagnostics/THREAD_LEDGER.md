...
|----|--------|------|--------|-------------|
| T6 | **Entry-location / exhaustion ("dead-cat") signal** — score rates structure, blind to "entering at the spent extreme." Merges old T7. | 3 | **VALIDATED (borderline) on clean data:** post-clamp n=108, exh_bb −0.21 / loc_bb +0.21 / EXH_composite −0.20 all CLEAR the ±0.19 floor AND beat every confluence factor — and STRENGTHENED post-clamp while confounds (pullback_probability −0.21→−0.05) died. Modest (~4% var), barely above floor, proxies only. | (1) Instrument: log real detect_pullback_setup inputs (extension_pct, volume_exhausted, "favors-opposite" flag) per signal. (2) Accumulate clean sessions (shrink floor). (3) If it holds → wire SYMMETRIC entry penalty (not veto), flag + re-baseline + symmetry-guard + §16. |
