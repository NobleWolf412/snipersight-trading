...
| T14 | **Maker-execution experiment — THE consequential lever** (edge verdict T8: only maker fills flip net-negative→positive) | strategic | DESIGN ENTRY done (decisions/2026-06-06__maker-execution-experiment-design.md, §15, paper-only). Maker path ALREADY EXISTS (pending machinery + monitor-loop fill + TTL); only the snap + immediate-fill force taker. Phase 1 = tiny default-off `execution_mode` toggle (skip snap + skip immediate fill → rest at the OB) measuring FILL RATE. | **AWAITING OPERATOR GO for Phase 1.** GATE 1: if fill rate collapses / adverse selection (resting sell-limits miss winners in downtrends, fill reversals) → STOP = strategy verdict #3 (rethink premise). Phase 2 (maker/taker fee model + edge_significance maker scenario) gated on GATE 1. HARD: forbid rest_maker+use_testnet (live-bleed at paper_trading_service.py:485-492); live_executor untouched. Honest prior: likely fewer trades, similar/worse net. |
