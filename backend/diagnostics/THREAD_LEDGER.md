...
| T14 | **Maker-execution experiment — THE consequential lever** (edge verdict T8: only maker fills flip net-negative→positive) | strategic | DESIGN ENTRY done (decisions/2026-06-06__maker-execution-experiment-design.md, §15, paper-only). Maker path ALREADY EXISTS (pending machinery + monitor-loop fill + TTL); only the snap + immediate-fill force taker. Phase 1 = tiny default-off `execution_mode` toggle (skip snap + skip immediate fill → rest at the OB) measuring FILL RATE. | **PHASE 1 SHIPPED** (default-off `execution_mode` toggle): config field + `rest_maker_active` property (testnet-guarded) + snap-skip + immediate-fill-skip → rests at the OB; tagged on signals.jsonl + CompletedTrade; API plumbing (PaperTradingConfigRequest.execution_mode) so it's botConfig-reachable. snap_taker DEFAULT byte-inert (proven); backend-integrity CLEAN; §16 (regression test_maker_execution_toggle.py covers default-inert + testnet-forced-off). **GATE 1 RESULT (1 rest_maker session, fill_rate.py):** fill rate **26%** (9/34 — LOW, as predicted; resting limits miss ~3/4 of setups). BUT the 9 fills were NOT adverse-selected (the predicted failure mode): 78% win, +5.33/tr gross vs snap_taker 50%/−2.15 — OB-resting entries that filled look better-located. **n=9 = noise-range; only the 26% fill rate is solid.** AMBIGUOUS: fill-rate-collapse confirmed (cost real), adverse-selection refuted (benefit maybe real, unproven). **NEXT: accumulate more rest_maker sessions → re-read at n≈40.** If better win% HOLDS at higher n → maker viable despite low fill (maker fees ≈ 0, gross≈net) = first real edge. If regresses to ~50% → 9-trade mirage, low fill kills it → verdict #3. Phase 2 (fee model) gated on that. live_executor untouched. |
