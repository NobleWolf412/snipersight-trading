...
class PositionSizer:
    ...
    def calculate_fixed_fractional(
        ...
        if not 0 < risk_pct <= self.max_risk_pct:
            raise ValueError(f"Risk % must be 0-{self.max_risk_pct}, got {risk_pct}")
        if entry_price <= 0:
    ...
    def calculate_kelly(
        ...
        result = self.calculate_fixed_fractional(
            risk_pct=risk_pct, entry_price=entry_price, stop_price=stop_price, leverage=leverage
        )
    ...
    def calculate_atr_based(
        ...
        result.metadata.update(
            {"atr": atr, "atr_multiplier": atr_multiplier, "stop_distance_atr": atr_multiplier}
        )

        return result

    def calculate_fixed_dollar_risk(
        self, risk_amount: float, entry_price: float, stop_price: float, leverage: float = 1.0
    ) -> PositionSize:
        """
    ...
    def validate_position_size(
        ...
        if notional_value < self.min_order_value:
            return (
                False,
                f"Position value ${notional_value:.2f} below minimum ${self.min_order_value:.2f}",
            )
