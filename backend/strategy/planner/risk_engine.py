...
def _calculate_stop_loss(
    ...
    if consolidation_for_stop:
        ...
        if not is_quality_consolidation:
            ...
            if is_bullish:
                ...
                stop_level = max(entry_zone.far_entry * 0.001, consolidation_for_stop.low - (stop_buffer * atr))
    ...
    if is_swing_mode and multi_tf_data and current_price:
        ...
        if swing_result:
            ...
            if is_bullish:
                stop_level = max(entry_zone.far_entry * 0.001, swing_level - (stop_buffer * atr))
                distance_atr = (entry_zone.far_entry - stop_level) / atr
            else:
    ...
    if is_bullish:
        ...
        if valid_stops:
            ...
            stop_level = max(entry_zone.far_entry * 0.001, stop_level - (stop_buffer * atr))  # Dynamic regime-aware buffer beyond structure
            ...
            if entry_obs:
                ...
                stop_level = max(entry_zone.far_entry * 0.001, best_entry_ob.low - (stop_buffer * atr))
                ...
                if swing_level:
                    stop_level = max(entry_zone.far_entry * 0.001, swing_level - (
                        stop_buffer * atr
                    ...
                    logger.warning(
                        f"⚠️ Using ATR-based fallback stop: {stop_level:.4f} ({fallback_atr_mult}x ATR) for {mode_profile} mode"
                    )
...
def _scale_stop_for_leverage(
    stop_level: float, entry_price: float, leverage: int, is_bullish: bool
) -> tuple[float, dict]:
    """
    Scale stop loss distance based on leverage tier.
    Lower leverage = Wider stop (Extended tolerance)
    Higher leverage = Tighter stop (Priority on position size)

    This ensures R:R remains consistent with target scaling.
    """
    if leverage <= 1:
        return stop_level, {}

    # Determine scaling factor based on leverage tier (same as targets)
    if leverage <= 5:
        scale_factor = 1.2  # Extended - can hold for bigger moves/more room
        tier = "extended"
    elif leverage <= 10:
        scale_factor = 1.0  # Standard - no adjustment
        tier = "standard"
    elif leverage <= 25:
        scale_factor = 0.75  # Tight - higher precision required
        tier = "tight"
    else:
        scale_factor = 0.5  # Very tight - aggressive scalp precision
        tier = "very_tight"

    if scale_factor == 1.0:
        return stop_level, {"tier": tier, "scale_factor": scale_factor}

    original_distance = abs(stop_level - entry_price)
    ...
    # Safety clamp: Ensure stop stays positive
    new_level = max(entry_price * 0.001, new_level)

    logger.info(
        f"🔧 LEVERAGE STOP ADJ: {leverage}x | Scale={scale_factor:.2f} | Tier={tier} | {stop_level:.4f} → {new_level:.4f}"
    )
    ...
    adjustment_meta = {
        ...
        "original_stop": stop_level,
        "adjusted_stop": new_level,
    }

    return new_level, adjustment_meta
...
def _calculate_swing_structural_targets(
    ...
    if not targets:
        ...
        for mult in [2.0, 3.5, 5.5]:
            ...
            level = avg_entry + dist if is_bullish else max(avg_entry * 0.001, avg_entry - dist)
            targets.append(
                Target(
                    level=level,
...
def _adjust_targets_for_leverage(
    ...
    for target in targets:
        ...
        if is_bullish:
            ...
            # For shorts, "extended" targets (>1.0x move) can push price below zero
            # Ensure price stays positive (min 0.1% of entry price)
            new_level = max(entry_price * 0.001, entry_price - adjusted_distance)
