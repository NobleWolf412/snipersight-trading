...
def calculate_confluence_score(
                ...
                try:
                    ...
                    for div in divergences.get("rsi", []):
                        if div.is_reversal():
                            ...
                            divergence_rationale.append(
                                f"RSI {div.divergence_type.replace('_', ' ').title()} ({div.strength:.0f}%)"
                            )
                        else:
                    ...
                    for div in divergences.get("macd", []):
                        if div.is_reversal():
                            ...
                            divergence_rationale.append(
                                f"MACD {div.divergence_type.replace('_', ' ').title()} ({div.strength:.0f}%)"
                            )
                        else:
    ...
    if current_price and smc_snapshot.swing_structure:
        try:
            ...
            for fib_tf in fib_timeframes:
                ...
                swing_high = (
                    ss.get("last_hh") or ss.get("last_lh")
                    if isinstance(ss, dict)
                    else getattr(ss, "last_hh", None) or getattr(ss, "last_lh", None)
                )
                swing_low = (
                    ss.get("last_ll") or ss.get("last_hl")
                    if isinstance(ss, dict)
                    else getattr(ss, "last_ll", None) or getattr(ss, "last_hl", None)
                )
                ...
                if swing_high and swing_low and swing_high > swing_low:
                    ...
                    if nearest_fib:
                        ...
                        if proximity_pct <= 0.5 and is_key_level:
                            ...
                            fib_score = 60.0
                            fib_rationale = (
                                f"Price approaching {fib_tf} {nearest_fib.ratio*100:.1f}% Fib"
                            )
                        else:
            ...
            if not fib_levels_calculated:
                logger.info(
                    f"📐 Fibonacci DIAGNOSTIC [{symbol}]: No valid HTF swings found. Checked timeframes: {fib_timeframes}. "
            ...
            logger.warning(
                f"📐 Fibonacci DIAGNOSTIC [{symbol}]: Scoring FAILED with {error_type}: {str(e)[:150]}"
            )
    ...
    logger.warning(
        "⚠️ VARIANCE CURVE DISABLED - Raw score: %.1f (diagnosing score distribution)", raw_score
    )
...
def _score_order_blocks(order_blocks: List[OrderBlock], direction: str) -> float:
    ...
    if not aligned_obs:
        ...
        logger.info(
            "📦 OB Score: 0 aligned (looking for %s) | %d total | %d %s OBs exist",
            normalized_dir,
            len(order_blocks),
            opposite_count,
            opposite_dir,
    ...
    logger.debug(
        "📦 OB Score: %d aligned %s OBs out of %d total",
        len(aligned_obs),
        normalized_dir,
        len(order_blocks),
    )
    ...
    logger.debug(
        "📦 OB Score: %.1f (base=%.1f [fresh=%.1f disp=%.1f mitig=%.1f@%.0f%%], grade=%s[%.1f], tf=%s[%.1f])",
        score,
        base_score,
        best_ob.freshness_score * 30,
        min(best_ob.displacement_strength / 2.0, 1.0) * 25,
        mitigation_score,
        best_ob.mitigation_level * 100,
        getattr(best_ob, "grade", "B"),
        grade_weight,
        getattr(best_ob, "timeframe", "?"),
        tf_weight,
    )
...
def _score_fvgs(fvgs: List[FVG], direction: str) -> float:
    ...
    if not aligned_fvgs:
        ...
        logger.info(
            "🔲 FVG Score: 0 aligned (looking for %s) | %d total | %d %s FVGs exist",
            normalized_dir,
            len(fvgs),
            opposite_count,
            opposite_dir,
    ...
    logger.debug(
        "🔲 FVG Score: %d aligned %s FVGs out of %d total",
        len(aligned_fvgs),
        normalized_dir,
        len(fvgs),
    )
    ...
    if not unfilled:
        ...
        best_filled = max(
            aligned_fvgs, key=lambda fvg: fvg.size * _get_grade_weight(getattr(fvg, "grade", "B"))
        )
