...
def detect_cycle_context(
    ...
    if len(df) < config.wcl_max_days:
        logger.debug(
            "Insufficient data for cycle detection (need %d days, have %d)",
            config.wcl_max_days,
            len(df),
        )
...
def _detect_dcl(df: pd.DataFrame, config: CycleConfig) -> dict:
    ...
    for i in range(lookback, len(recent_df) - lookback):
        ...
        if is_swing:
            swing_lows.append({"idx": i, "price": current_low, "timestamp": recent_df.index[i]})
    ...
    # Calculate days since the candidate swing low FIRST (needed for timing gate)
    days_since = (df.index[-1] - last_swing["timestamp"]).days

    # ── FIX 1: DCL TIMING GATE ───────────────────────────────────────────────
    # dcl_early_zone (default 15) is the minimum number of days that must pass
    # after a swing low before it can be labelled a confirmed DCL.
    # Without this check, any swing bounce on day 5-14 gets confirmed —
    # that was causing ~43% of all bars to carry a false CONFIRMED DCL label.
    if days_since < config.dcl_early_zone:
        return {
            "price": last_swing["price"],
            "timestamp": (
                last_swing["timestamp"].to_pydatetime()
                if hasattr(last_swing["timestamp"], "to_pydatetime")
                else last_swing["timestamp"]
            ),
            "days_since": days_since,
            "confirmation": CycleConfirmation.UNCONFIRMED,  # Too early to confirm
        }
    # ─────────────────────────────────────────────────────────────────────────
...
def _detect_wcl(df: pd.DataFrame, config: CycleConfig, dcl_info: dict) -> dict:
    ...
    - FIX 2a: Timing gate (wcl_early_zone) before confirming
    - FIX 2b: Use most-recent valid swing low within the WCL window,
              NOT the all-time deepest (which gets anchored to old crashes)
    - FIX 2c: Staleness guard — WCL older than wcl_max_days * 2 is ignored
    ...
    # ── FIX 2c: Staleness window ─────────────────────────────────────────────
    # Only look back wcl_max_days * 2 (default 100 bars) for the WCL candidate.
    # A WCL older than this is stale and should trigger a fresh cycle search.
    staleness_limit = config.wcl_max_days * 2
    analysis_window = min(len(df), staleness_limit)
    recent_df = df.tail(analysis_window)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # ── FIX 2b: Use most-recent valid swing low, not deepest ─────────────────
    # Original code used min(swing_lows, key=price) which anchored to the
    # deepest historical crash and never reset. Instead, prefer the most
    # recent swing low that is within 1.5x the max WCL window (~75 bars).
    # This allows the WCL to reset naturally after each cycle.
    valid_window = int(config.wcl_max_days * 1.5)
    valid_lows = [
        s for s in swing_lows
        if (len(recent_df) - 1 - s["idx"]) <= valid_window
    ]
    # If no lows in the valid window, fall back to deepest in full window
    if valid_lows:
        # Among valid candidates, prefer the lowest (most significant WCL)
        wcl_candidate = min(valid_lows, key=lambda x: x["price"])
    else:
        wcl_candidate = min(swing_lows, key=lambda x: x["price"])
    # ─────────────────────────────────────────────────────────────────────────

    # Days since the WCL candidate
    days_since = (df.index[-1] - wcl_candidate["timestamp"]).days

    # ── FIX 2a: WCL TIMING GATE ──────────────────────────────────────────────
    # Same principle as DCL: if the candidate is too recent, it cannot be
    # a confirmed WCL yet. wcl_early_zone (default 30 days) is the minimum.
    if days_since < config.wcl_early_zone:
        return {
            "price": wcl_candidate["price"],
            "timestamp": (
                wcl_candidate["timestamp"].to_pydatetime()
                if hasattr(wcl_candidate["timestamp"], "to_pydatetime")
                else wcl_candidate["timestamp"]
            ),
            "days_since": days_since,
            "confirmation": CycleConfirmation.UNCONFIRMED,  # Too early to confirm
        }
...
def _detect_cycle_high(df: pd.DataFrame, dcl_info: dict, config: CycleConfig) -> dict:
    ...
    if dcl_ts is None:
        ...
        return {
            ...
            "timestamp": (
                high_idx.to_pydatetime() if hasattr(high_idx, "to_pydatetime") else high_idx
            ),
            "days_since_dcl": None,
        }
...
def _calculate_translation(
    dcl_info: dict, cycle_high_info: dict, config: CycleConfig
) -> CycleTranslation:
    """
...
def should_boost_direction(cycle_context: CycleContext, direction: str) -> Tuple[bool, float]:
    """
...
def should_bypass_htf_alignment(
    cycle_context: CycleContext, structural_break: Optional[StructuralBreak] = None
) -> bool:
    """
    ...
    if cycle_context.is_at_cycle_high and cycle_context.translation == CycleTranslation.LTR:
        return True
