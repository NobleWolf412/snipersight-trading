...
class CycleState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expected_length": {"min": self.expected_length[0], "max": self.expected_length[1]},
            "midpoint": self.midpoint,
            "cycle_low": {
                "price": self.cycle_low_price,
                "bar": self.cycle_low_bar,
                "timestamp": (
                    self.cycle_low_timestamp.isoformat() if self.cycle_low_timestamp else None
                ),
            },
            "cycle_high": {
...
def detect_symbol_cycles(
    df: pd.DataFrame, symbol: str = "UNKNOWN", current_price: Optional[float] = None
) -> SymbolCycles:
    """
...
def _detect_cycle(
    df: pd.DataFrame, cycle_type: Literal["DCL", "WCL"], current_price: float
) -> CycleState:
    """
    ...
    translation = _determine_translation(
        translation_pct, bars_since_low=bars_since_low, midpoint=midpoint, cycle_type=cycle_type
    )
...
def _find_swing_lows(df: pd.DataFrame, lookback: int = 3) -> List[Dict]:
    ...
    for i in range(lookback, len(df) - lookback):
        ...
        if is_swing:
            swing_lows.append(
                {"price": float(current_low), "index": i, "bars_ago": len(df) - 1 - i}
            )
...
def _determine_translation(
    translation_pct: float, bars_since_low: int = 0, midpoint: float = 0, cycle_type: str = "DCL"
) -> Translation:
    """
...
def _determine_status(
    translation: Translation, is_failed: bool, bars_since_low: int, min_length: int
) -> CycleStatus:
...
def _determine_bias(
    translation: Translation, is_failed: bool, is_in_window: bool
...
def _assess_cycles(
    dcl: CycleState, wcl: CycleState
) -> Tuple[
    Literal["LONG", "SHORT", "NEUTRAL"], Literal["ALIGNED", "MIXED", "CONFLICTING"], List[str]
]:
    """
...
def _empty_cycle_state(
    cycle_type: str, min_len: int, max_len: int, current_price: float
) -> CycleState:
...
def get_cycle_context_for_signal(
    cycles: SymbolCycles, signal_direction: Literal["LONG", "SHORT"]
) -> Dict[str, Any]:
    """
    ...
    dcl_aligned = (signal_direction == "LONG" and dcl.bias in ["LONG", "NEUTRAL"]) or (
        signal_direction == "SHORT" and dcl.bias in ["SHORT", "NEUTRAL"]
    )
    wcl_aligned = (signal_direction == "LONG" and wcl.bias in ["LONG", "NEUTRAL"]) or (
