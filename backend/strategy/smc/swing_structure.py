...
def _determine_trend(swings: List[SwingPoint], current_price: Optional[float] = None) -> TrendState:
    """
    Determine trend from recent swing structure with recency weighting.
    
    IMPROVED: 
    1. Weights most recent swings more heavily (2x weight for last 2 swings)
    2. Checks if current price is making new LL/HH beyond last swing
    3. Uses stricter threshold to avoid false bullish in downtrends
    """
    if len(swings) < 4:
        return "neutral"

    # Look at last 8 swings (increased from 6)
    recent = swings[-8:] if len(swings) >= 8 else swings[-6:]
    
    # Find last swing high and low for LL/HH detection
    last_swing_low = None
    last_swing_high = None
    for s in reversed(swings):
        if last_swing_low is None and not s.is_high:
            last_swing_low = s
        if last_swing_high is None and s.is_high:
            last_swing_high = s
        if last_swing_low and last_swing_high:
            break
    
    # Check if current price is making a new extreme
    if current_price is not None:
        # If price is below last swing low by >1%, it's making a new LL
        if last_swing_low and current_price < last_swing_low.price * 0.99:
            logger.info(
                f"📉 TREND OVERRIDE: Price ${current_price:.2f} below last swing low "
                f"${last_swing_low.price:.2f} - confirming BEARISH"
            )
            return "bearish"
        
        # If price is above last swing high by >1%, it's making a new HH  
        if last_swing_high and current_price > last_swing_high.price * 1.01:
            logger.info(
                f"📈 TREND OVERRIDE: Price ${current_price:.2f} above last swing high "
                f"${last_swing_high.price:.2f} - confirming BULLISH"
            )
            return "bullish"
    
    # === RECENCY-WEIGHTED SCORING ===
    # Last 2 swings get 2x weight, last 3-4 get 1.5x weight, rest get 1x
    bullish_score = 0.0
    bearish_score = 0.0
    
    for i, s in enumerate(recent):
        # Recency weight: last 2 get 2x, next 2 get 1.5x, rest get 1x
        recency_index = len(recent) - 1 - i  # 0 = oldest, len-1 = newest
        if recency_index >= len(recent) - 2:
            weight = 2.0  # Last 2 swings
        elif recency_index >= len(recent) - 4:
            weight = 1.5  # Swings 3-4 from end
        else:
            weight = 1.0  # Older swings
            
        if s.swing_type == "HH":
            bullish_score += weight
        elif s.swing_type == "HL":
            bullish_score += weight
        elif s.swing_type == "LH":
            bearish_score += weight
        elif s.swing_type == "LL":
            bearish_score += weight
    
    # Log the scoring breakdown
    logger.debug(
        f"Swing trend scoring: bullish={bullish_score:.1f} vs bearish={bearish_score:.1f} "
        f"(from {len(recent)} swings)"
    )
    
    # Require 1.5 point lead (more strict than before)
