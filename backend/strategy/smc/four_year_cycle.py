...
class FourYearCycleContext:
    ...
    def suggests_long(self) -> bool:
        ...
        return (
            self.macro_bias == "BULLISH" or
            self.phase in [FYCPhase.ACCUMULATION, FYCPhase.MARKUP]
        )
    ...
    def suggests_short(self) -> bool:
        ...
        return (
            self.macro_bias == "BEARISH" or
            self.phase in [FYCPhase.DISTRIBUTION, FYCPhase.MARKDOWN]
        )
...
HISTORICAL_4YC_LOWS = [
    {
        ...
        "price": 2.0,
        "event": "Post-Mt.Gox hack bottom"
    },
    {
        "date": date(2015, 1, 14),
        "price": 170.0,
        "event": "Post-2014 halving capitulation"
    },
    {
        "date": date(2018, 12, 15),
        "price": 3200.0,
        "event": "Crypto winter bottom"
    },
    {
        "date": date(2022, 11, 21),
        "price": 15476.0,
        "event": "FTX collapse capitulation"
    },
]
...
def get_four_year_cycle_context(
    ...
    custom_anchor: Optional[Dict] = None
    ...
    logger.debug(
        ...
        days_since, position_pct, phase.value, macro_bias, confidence
    )
...
def get_fyc_confluence_modifier(
    fyc_context: FourYearCycleContext,
    direction: str
) -> tuple[float, str]:
    """
...
def get_halving_info() -> Dict:
    ...
    return {
        ...
        "halving_history": [
            {"date": h.isoformat(), "number": i+1} 
            for i, h in enumerate(BTC_HALVINGS)
        ]
    }
