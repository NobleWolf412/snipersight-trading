...
def _has_structure_context(
    df: pd.DataFrame, candle_idx: int, direction: str, atr_value: float, swing_lookback: int = 20
) -> tuple:
    """
...
def _has_bos_confirmation(
    df: pd.DataFrame, candle_idx: int, direction: str, lookback: int, min_candles_after: int = 2
) -> bool:
    """
...
def detect_order_blocks(
    df: pd.DataFrame, config: SMCConfig | dict | None = None
) -> List[OrderBlock]:
    """
    ...
    if len(df) < lookback_candles + 20:  # Need enough data for ATR calculation
        raise ValueError(
            f"DataFrame too short for order block detection (need {lookback_candles + 20} rows, got {len(df)})"
        )
    ...
    for i in range(
        20, len(df) - lookback_candles
    ):  # Start after ATR warmup, leave room for displacement check
        candle = df.iloc[i]
        ...
        if (lower_wick / body >= min_wick_ratio) or (is_engulfing and body > min_body):
            ...
            # Volume confirmation
            volume_spike = (
                candle["volume"] > (avg_volume.iloc[i] * volume_threshold)
...
def filter_overlapping_order_blocks(
    order_blocks: List[OrderBlock], max_overlap: float = 0.5
) -> List[OrderBlock]:
    """
...
def detect_order_blocks_structural(
    ...
    for close_idx in range(1, n):
        ...
        if _close[close_idx] > last_swing_high_val and last_swing_high_ts not in crossed_highs:
            ...
            atr_val = (
                atr.iloc[close_idx] if close_idx < len(atr) and pd.notna(atr.iloc[close_idx]) else 1
            )
            disp_atr = displacement / atr_val if atr_val > 0 else 0
...
def update_ob_lifecycle(
    df: pd.DataFrame, order_blocks: List[OrderBlock], preset: str = "defaults"
) -> List[OrderBlock]:
    """
...
def filter_to_active_obs(
    ...
    for ob in order_blocks:
        ...
        if ob.mitigation_level > max_mitigation:
            logger.debug(
                "🚫 OB filtered (mitigation %.1f%% > %.1f%%): %s @ %s",
                ob.mitigation_level * 100,
                max_mitigation * 100,
                ob.direction,
                ob.timestamp,
            )
        ...
        if require_structure_confirmation and structure_breaks:
            ...
            if not confirmed:
                logger.debug(
                    "🚫 OB filtered (no structure confirmation): %s @ %s",
                    ob.direction,
                    ob.timestamp,
                )
                continue
    ...
    logger.info(
        "🎯 OB Filter: %d/%d OBs are active (structure-confirmed + fresh)",
        len(active_obs),
        len(order_blocks),
    )
...
def _check_structure_confirmation(
    ob: OrderBlock, structure_breaks: List, df: pd.DataFrame, window_candles: int = 10
) -> bool:
    """
