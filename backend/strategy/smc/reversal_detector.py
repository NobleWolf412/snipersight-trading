...
def detect_reversal_context(
    ...
    if long_reversal.is_reversal_setup and short_reversal.is_reversal_setup:
        return long_reversal if long_reversal.confidence >= short_reversal.confidence else short_reversal
...
def detect_long_reversal(
    ...
    low_sweeps = [s for s in smc_snapshot.liquidity_sweeps 
                  if s.sweep_type == "low" and s.confirmation]
...
def detect_short_reversal(
    ...
    if (cycle_context.cycle_midpoint_price and 
        cycle_context.dcl_price and
        indicators and hasattr(indicators, 'close')):
    ...
    high_sweeps = [s for s in smc_snapshot.liquidity_sweeps 
                   if s.sweep_type == "high" and s.confirmation]
...
def _find_bullish_choch(structural_breaks: List[StructuralBreak]) -> Optional[StructuralBreak]:
    ...
    """
    chochs = [sb for sb in structural_breaks if sb.break_type == "CHoCH"]
    
    if not chochs:
        return None
    
    # Get most recent CHoCH
    # Note: In the current model, CHoCH doesn't store direction explicitly
    # We would need context to determine if it's bullish or bearish
    # For now, return the most recent CHoCH
    latest = max(chochs, key=lambda c: c.timestamp)
    
    # In a full implementation, we'd check the trend before the CHoCH
    # to determine if it's bullish (from downtrend) or bearish (from uptrend)
    return latest
...
def _find_bearish_choch(structural_breaks: List[StructuralBreak]) -> Optional[StructuralBreak]:
    ...
    # Similar to bullish - would need trend context for proper direction
    latest = max(chochs, key=lambda c: c.timestamp)
    return latest
...
def combine_reversal_with_cycle_bonus(
    ...
    if reversal_context.direction == "LONG":
        if (reversal_context.choch_detected and 
            reversal_context.cycle_aligned and
        ...
        if (reversal_context.liquidity_swept and
    ...
    if reversal_context.direction == "SHORT":
        if (reversal_context.choch_detected and
            cycle_context.translation == CycleTranslation.LTR and
            cycle_context.phase in [CyclePhase.DISTRIBUTION, CyclePhase.MARKDOWN]):
            bonus += 15.0
