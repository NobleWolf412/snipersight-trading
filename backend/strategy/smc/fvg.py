...
def detect_fvgs(
    ...
    if len(df) < 20:  # Need minimum data for ATR
        raise ValueError(
            f"DataFrame too short for FVG detection (need at least 20 rows, got {len(df)})"
        )
    ...
    if potential_bullish_gaps > 0 or potential_bearish_gaps > 0:
        logger.info(
            ...
            bullish_size_fails,
            potential_bearish_gaps,
            bearish_overlap_fails,
            bearish_size_fails,
            len(fvgs),
        )
...
def merge_consecutive_fvgs(
    fvgs: List[FVG], max_gap_atr: float = 0.5, atr_value: float = 1.0
) -> List[FVG]:
    """
    ...
    for next_fvg in sorted_fvgs[1:]:
        ...
        if gap <= max_gap:
            ...
            best_grade = (
                "A"
                if ("A" in [current.grade, next_fvg.grade])
                else ("B" if "B" in [current.grade, next_fvg.grade] else "C")
            )

            current = replace(
                current, top=new_top, bottom=new_bottom, size=new_size, grade=best_grade
            )
        else:
