...
def get_session_info(df: pd.DataFrame, timestamp: Optional[datetime] = None) -> SessionInfo:
    """
...
def filter_candles_in_kill_zone(df: pd.DataFrame, kill_zone: KillZone) -> pd.DataFrame:
    """
    ...
    mask = df.index.to_series().apply(lambda ts: _time_in_range(ts.time(), (sh, sm), (eh, em)))
