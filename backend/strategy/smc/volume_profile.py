...
def analyze_entry_volume_context(
    entry_price: float, volume_profile: VolumeProfile, direction: str
) -> Dict:
    """
...
def find_volume_zones(
    volume_profile: VolumeProfile, direction: str, current_price: float
) -> Dict[str, List[float]]:
    """
    ...
    return {"support": support_zones[:5], "resistance": resistance_zones[:5]}  # Top 5 nearest
...
def calculate_volume_confluence_factor(
    ...
    rationale_parts = [f"Volume Profile Analysis:", context["rationale"]]
