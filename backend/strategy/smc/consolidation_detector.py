...
from backend.shared.models.smc import Consolidation, FVG
...
def detect_consolidations(
    ...
    for i in range(window_size, len(df) - 5):  # Leave room for breakout confirmation
        ...
        if breakout_data:
            ...
            logger.debug(f"Detected consolidation on {timeframe}: {touches} touches, strength={strength:.2f}, breakout={breakout_direction}")
...
def _detect_breakout(
    ...
    if not _confirm_breakout_hold(future_window, breakout_candle + 1, high_level, low_level, breakout_direction):
        return None
