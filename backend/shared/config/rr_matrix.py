...
def get_rr_threshold(
    plan_type: Literal["SMC", "ATR_FALLBACK", "HYBRID"], mode_profile: Optional[str] = None
) -> RRThreshold:
    """
...
def validate_rr(
    ...
    if (
        ...
        return (
            True,
            f"EV override: R:R {risk_reward:.2f} < {effective_min_rr:.2f} but EV={expected_value:.3f} with {confluence_score:.1f}% confluence",
        )
