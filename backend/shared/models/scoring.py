...
class ConfluenceBreakdown:
    ...
    def __post_init__(self):
        ...
        if not 0 <= self.synergy_bonus <= 50:
            raise ValueError(f"Synergy bonus must be 0-50, got {self.synergy_bonus}")
        if not 0 <= self.conflict_penalty <= 50:
            raise ValueError(f"Conflict penalty must be 0-50, got {self.conflict_penalty}")
