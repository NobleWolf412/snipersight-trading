...
class IndicatorSnapshot:
    ...
    volume_is_accelerating: Optional[bool] = (
        None  # True if acceleration > threshold + min consecutive
    )
    volume_accel_direction: Optional[str] = (
        None  # 'bullish', 'bearish', 'neutral' - direction of volume+price
    )
    volume_exhaustion: Optional[bool] = (
        None  # True if volume declining from recent spike (reversal signal)
    )
    ...
    def __post_init__(self):
        ...
        for tf, snapshot in self.by_timeframe.items():
            if not isinstance(snapshot, IndicatorSnapshot):
                raise TypeError(
                    f"Timeframe '{tf}' must contain IndicatorSnapshot, " f"got {type(snapshot)}"
                )
