...
def test_validation_suite():
    ...
    for validation in results['validations']:
        ...
        if 'max_pct_diff' in validation:
            ...
            print(f"    Tolerance: {validation.get('tolerance_pct', validation.get('tolerance', 0))}%")
        else:
...
def test_safe_computation():
    ...
    try:
        ...
        print(f"✓ BBands computed: Upper={upper.iloc[-1]:.2f}, Mid={mid.iloc[-1]:.2f}, Lower={lower.iloc[-1]:.2f}")
    except Exception as e:
