...
import pytest
from backend.strategy.planner.regime_engine import _calculate_htf_bias_factor
...
def test_htf_bias_at_max_distance():
    ...
    assert factor_at_max == pytest.approx(
        1.0, abs=0.01
    ...
    assert factor_beyond == pytest.approx(
        1.0, abs=0.01
...
def test_htf_bias_linear_interpolation():
    ...
    assert factor_mid == pytest.approx(
        0.6, abs=0.01
    ...
    assert factor_quarter == pytest.approx(
        0.4, abs=0.01
    ...
    assert factor_three_quarters == pytest.approx(
        0.8, abs=0.01
