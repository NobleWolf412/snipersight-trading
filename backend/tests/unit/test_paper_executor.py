...
def test_place_limit_order():
    ...
    with pytest.raises(ValueError, match="Limit orders require a price"):
        executor.place_order(symbol="BTC/USDT", side="BUY", order_type="LIMIT", quantity=0.1)
...
def test_place_order_validation():
    ...
    with pytest.raises(ValueError, match="Quantity must be positive"):
        executor.place_order(symbol="BTC/USDT", side="BUY", order_type="MARKET", quantity=0)
...
def test_execute_market_buy_order():
    ...
    executor = PaperExecutor(
        initial_balance=10000, fee_rate=0.001, slippage_bps=5, enable_partial_fills=False
    )
...
def test_fee_calculation():
    ...
    executor = PaperExecutor(
        initial_balance=10000, fee_rate=0.001, enable_partial_fills=False  # 0.1%
    )
...
def test_slippage_calculation():
    ...
    executor = PaperExecutor(
        initial_balance=10000, slippage_bps=10, enable_partial_fills=False  # 10 basis points = 0.1%
    )
...
def test_average_fill_price():
    ...
    expected_avg = (fill1.quantity * fill1.price + fill2.quantity * fill2.price) / (
