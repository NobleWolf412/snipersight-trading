...
def confluence():
    return ConfluenceBreakdown(
        total_score=75.0,
        factors=[
            ConfluenceFactor(name="Structure", score=80, weight=0.4, rationale="OB + FVG confluence"),
...
@pytest.mark.parametrize("atr,current_price,expected_label", [
    (0.2, 100.0, "calm"),
    (0.8, 100.0, "normal"),
    (1.5, 100.0, "elevated"),
    (3.0, 100.0, "explosive"),
])
def test_atr_regime_classification(monkeypatch, base_config, empty_smc, confluence, atr, current_price, expected_label):
    ...
    def fake_entry_zone(*args, **kwargs):
        ...
        return EntryZone(near_entry=current_price, far_entry=current_price * 0.995, rationale="Test"), True
    ...
    assert regime_meta["label"] == expected_label, f"Expected {expected_label}, got {regime_meta['label']}"
...
def test_alt_stop_suggested_high_liq_risk(monkeypatch, base_config, empty_smc, confluence):
    ...
    assert alt_stop["level"] < plan.stop_loss.level, "Suggested alt stop should extend further (lower for long)"

    # Verify telemetry event emitted
    telemetry = get_telemetry_logger()
    events = telemetry.get_cached_events(limit=10)
    assert any(e["event_type"] == EventType.ALT_STOP_SUGGESTED.value for e in events), "ALT_STOP_SUGGESTED event not logged"
...
def test_no_alt_stop_for_comfortable_liq(monkeypatch, base_config, empty_smc, confluence):
    ...
    assert plan.metadata.get("alt_stop") is None, "Alt stop should not be suggested for comfortable cushion"
    telemetry = get_telemetry_logger()
    events = telemetry.get_cached_events(limit=5)
    assert not any(e["event_type"] == EventType.ALT_STOP_SUGGESTED.value for e in events), "Alt stop event should not be logged"
