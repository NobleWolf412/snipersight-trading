...
def make_indicator_snapshot(
    ...
    snapshot.macd_histogram_series = macd_histogram_series or [0.0004, 0.0005, 0.0005, 0.0006, 0.0005]
    return snapshot
...
class TestMACDModeConfig:
    ...
    def test_all_profiles_have_config(self):
        ...
        expected_profiles = ["macro_surveillance", "balanced", "intraday_aggressive", "precision", "stealth_balanced"]
...
class TestEvaluateMACDForMode:
    ...
    def test_bullish_with_bullish_macd_primary_mode(self):
        ...
        assert "persistence" in str(result["reasons"]).lower() or "PRIMARY" in str(result["reasons"])
