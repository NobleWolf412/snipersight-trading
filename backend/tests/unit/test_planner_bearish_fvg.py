...
def test_bearish_fvg_entry_ordering():
    ...
    assert entry_zone.near_entry > entry_zone.far_entry, \
    ...
    print(f"✓ Bearish FVG ordering correct: near={entry_zone.near_entry:.2f} > far={entry_zone.far_entry:.2f}")
...
def test_bearish_fvg_overlap_filtering():
    ...
    print(f"✓ Overlap filtering works: used fresh FVG at {entry_zone.near_entry:.2f}, ignored filled one at 107")
...
def test_bearish_atr_fallback_ordering():
    ...
    print(f"✓ ATR fallback maintains ordering: near={entry_zone.near_entry:.2f} > far={entry_zone.far_entry:.2f}")
