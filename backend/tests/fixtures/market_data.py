...
def generate_ranging_ohlcv(
    periods: int = 100, base_price: float = 100.0, range_pct: float = 0.05
) -> List[OHLCV]:
    """
    ...
    for i in range(periods):
        ...
        open_price = current_price
        close_price = np.clip(current_price * (1 + price_change), lower_bound, upper_bound)
...
def generate_with_order_block(
    periods: int = 100, ob_position: int = 70, direction: str = "bullish"
) -> List[OHLCV]:
    """
    ...
    """
    candles = (
        generate_bullish_trend_ohlcv(periods - 20)
        if direction == "bullish"
        else generate_bearish_trend_ohlcv(periods - 20)
    )
...
def generate_with_fvg(
    periods: int = 100, fvg_position: int = 50, direction: str = "bullish"
) -> List[OHLCV]:
    """
    ...
    """
    candles = (
        generate_bullish_trend_ohlcv(periods)
        if direction == "bullish"
        else generate_bearish_trend_ohlcv(periods)
    )
...
def generate_multi_timeframe_data(symbol: str = "BTC/USDT") -> Dict[str, List[OHLCV]]:
    ...
    return {
        ...
        "1H": generate_with_order_block(
            720, ob_position=680, direction="bullish"
        ),  # 30 days hourly with OB
