        ...
        try:
            ...
            try:
                ...
                if cycle_df is not None:
                    ...
                    # ── BRIDGE: merge symbol_detector failure flags ───────────────
                    ...
                    try:
                        ...
                        # Use dataclasses.replace to avoid mutating frozen fields
                        import dataclasses
                        cycle_context = dataclasses.replace(
                            cycle_context,
            ...
            if is_directional_conflict:
                logger.warning(f"{symbol}: Handling directional conflict - extracting factors")
                ...
                # PRIORITY 1: Try exception attributes (ConflictingDirectionsException)
                bull_breakdown = getattr(e, 'bullish_breakdown', None)
                bear_breakdown = getattr(e, 'bearish_breakdown', None)
                
                if bull_breakdown is not None and bear_breakdown is not None:
                    logger.info(f"{symbol}: [SOURCE: exception] Found breakdowns on exception object")
                    bull_score = bull_breakdown.total_score
                    bear_score = bear_breakdown.total_score
                    bull_synergy = getattr(bull_breakdown, 'synergy_bonus', 0)
                    bull_conflict = getattr(bull_breakdown, 'conflict_penalty', 0)
                    bear_synergy = getattr(bear_breakdown, 'synergy_bonus', 0)
                    bear_conflict = getattr(bear_breakdown, 'conflict_penalty', 0)
                    
                    # Extract factors with weighted_contribution
                    bull_factors = [
                        {
                            "name": f.name,
                            "score": f.score,
                        ...
                        for f in bull_breakdown.factors
                    ]
                    bear_factors = [
                        {
                            "name": f.name,
                        ...
                        for f in bear_breakdown.factors
                    ]
                    logger.info(f"{symbol}: Extracted {len(bull_factors)} bull, {len(bear_factors)} bear factors from exception")
                
                # PRIORITY 2: Fallback to context.metadata (always populated before exception)
                if not bull_factors or not bear_factors:
                    alt_conf = context.metadata.get("alt_confluence", {})
                    if alt_conf:
                        logger.info(f"{symbol}: [SOURCE: context.metadata] Using alt_confluence fallback")
                        if not bull_score:
                            bull_score = alt_conf.get("long", 0.0)
                        if not bear_score:
                            bear_score = alt_conf.get("short", 0.0)
                        
                        # Get factors from alt_confluence and add weighted_contribution
                        raw_long = alt_conf.get("long_factors", [])
                        raw_short = alt_conf.get("short_factors", [])
                        
                        if raw_long and not bull_factors:
                            bull_factors = [
                                {
                                    **f,
                                    "weighted_contribution": f.get("score", 0) * f.get("weight", 1),
                                }
                                for f in raw_long
                            ]
                        if raw_short and not bear_factors:
                            bear_factors = [
                                {
                                    **f,
                                    "weighted_contribution": f.get("score", 0) * f.get("weight", 1),
                        ...
                        logger.info(f"{symbol}: Extracted {len(bull_factors)} bull, {len(bear_factors)} bear factors from metadata")
                
                # PRIORITY 3: Parse score from error message if still missing
                if bull_score == 0.0:
                    try:
                        import re
                    ...
                    self.telemetry.log_event(
                        create_signal_rejected_event(
                            ...
                            diagnostics={
                                ...
                                "bearish_score": bear_score,
                                "gap": gap,
                                "bull_factors_count": len(bull_factors),
                                "bear_factors_count": len(bear_factors),
                            },
                        )
                    )
