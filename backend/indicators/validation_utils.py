    ...
    if check_positive_volume and 'volume' in df.columns:
        ...
        if zero_volume > 0:
            ...
            if zero_pct > 50:
                ...
                result['warnings'].append(
                    f"Found {zero_volume} zero volume bars ({zero_pct:.1f}%)"
                )
...
def validate_series(
    ...
    if min_length is not None and len(series) < min_length:
        result['errors'].append(
            f"{name}: need {min_length} values, got {len(series)}"
        )
        result['valid'] = False
    ...
    if check_nan:
        ...
        if nan_count > 0:
            ...
            if nan_pct > 10:
                result['errors'].append(
                    f"{name} has {nan_count} NaN values ({nan_pct:.1f}%)"
                )
                result['valid'] = False
            else:
                result['warnings'].append(
    ...
    if check_positive:
        ...
        if non_positive > 0:
            result['errors'].append(
                f"{name} has {non_positive} non-positive values"
            )
            result['valid'] = False
