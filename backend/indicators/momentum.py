...
def compute_rsi(df: pd.DataFrame, period: int = 14, validate_input: bool = True) -> pd.Series:
    ...
    if len(df) < period + 1:
        raise ValueError(
            f"DataFrame too short for RSI calculation (need {period + 1} rows, got {len(df)})"
        )
...
def compute_stoch_rsi(
    ...
    if len(df) < min_length:
        raise ValueError(
            f"DataFrame too short for Stoch RSI (need {min_length} rows, got {len(df)})"
        )
...
def compute_mfi(df: pd.DataFrame, period: int = 14) -> pd.Series:
    ...
    if len(df) < period + 1:
        raise ValueError(
            f"DataFrame too short for MFI calculation (need {period + 1} rows, got {len(df)})"
        )
    ...
    try:
        ...
        if rsi.isna().all():
            ...
            results["warnings"].append(
                f"RSI has {rsi.isna().sum()} NaN values ({rsi.isna().sum()/len(rsi)*100:.1f}%)"
            )
