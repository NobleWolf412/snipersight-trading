...
from backend.indicators.validation_utils import validate_ohlcv
...
def compute_atr(df: pd.DataFrame, period: int = 14, validate_input: bool = True) -> pd.Series:
    ...
    if len(df) < period + 1:
        raise ValueError(
            f"DataFrame too short for ATR calculation (need {period + 1} rows, got {len(df)})"
        )

    # Validate input data quality
    if validate_input:
        validate_ohlcv(df, require_volume=False, min_rows=period + 1, raise_on_error=True)
...
def compute_realized_volatility(
    df: pd.DataFrame, window: int = 20, annualize: bool = True, periods_per_year: int = 365
) -> pd.Series:
    """
    ...
    if len(df) < window + 1:
        raise ValueError(
            f"DataFrame too short for volatility calculation (need {window + 1} rows, got {len(df)})"
        )
...
def compute_bollinger_bands(
    ...
    if len(df) < period:
        raise ValueError(
            f"DataFrame too short for Bollinger Bands (need {period} rows, got {len(df)})"
        )
...
def compute_keltner_channels(
    ...
    if len(df) < min_length:
        raise ValueError(
            f"DataFrame too short for Keltner Channels (need {min_length} rows, got {len(df)})"
        )
...
def validate_volatility_indicators(df: pd.DataFrame) -> dict:
    ...
    """
    results = {"valid": True, "errors": [], "warnings": [], "using_pandas_ta": PANDAS_TA_AVAILABLE}

    try:
        atr = compute_atr(df)
        results["atr_latest"] = (
            float(atr.iloc[-1]) if len(atr) > 0 and not pd.isna(atr.iloc[-1]) else None
        )
    ...
    try:
        ...
        results["volatility_latest"] = (
            float(vol.iloc[-1]) if len(vol) > 0 and not pd.isna(vol.iloc[-1]) else None
        )
    ...
    try:
        ...
        results["bb_width"] = (
            float((bb_upper.iloc[-1] - bb_lower.iloc[-1])) if len(bb_upper) > 0 else None
        )
