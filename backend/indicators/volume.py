...
def detect_volume_spike(
    ...
    if len(df) < lookback:
        raise ValueError(
            f"DataFrame too short for volume spike detection (need {lookback} rows, got {len(df)})"
        )
...
def detect_volume_acceleration(
    df: pd.DataFrame, lookback: int = 5, min_consecutive: int = 3, accel_threshold: float = 0.1
) -> dict:
    """
    ...
    if len(df) < lookback + 2:
        raise ValueError(
            f"DataFrame too short for acceleration detection (need {lookback + 2} rows, got {len(df)})"
        )
    ...
    is_accelerating = (
        normalized_slope > accel_threshold and consecutive_increases >= min_consecutive
    )
...
def compute_obv(df: pd.DataFrame) -> pd.Series:
    ...
    if len(df) < 2:
        raise ValueError(
            f"DataFrame too short for OBV calculation (need at least 2 rows, got {len(df)})"
        )
...
def compute_volume_profile(
    df: pd.DataFrame, price_bins: int = 50, lookback: int = None
) -> pd.DataFrame:
    """
    ...
    volume_profile = pd.DataFrame({"price_level": price_levels, "volume": volume_at_price})
...
def compute_relative_volume(df: pd.DataFrame, period: int = 20) -> pd.Series:
    ...
    if len(df) < period:
        raise ValueError(
            f"DataFrame too short for RVOL calculation (need {period} rows, got {len(df)})"
        )
...
def validate_volume_indicators(df: pd.DataFrame) -> dict:
    ...
    try:
        obv = compute_obv(df)
        results["obv_latest"] = (
            float(obv.iloc[-1]) if len(obv) > 0 and not pd.isna(obv.iloc[-1]) else None
        )
    ...
    try:
        ...
        results["rvol_latest"] = (
            float(rvol.iloc[-1]) if len(rvol) > 0 and not pd.isna(rvol.iloc[-1]) else None
        )
