...
class RegimeDetector:
    ...
    def analyze_timeframe_trend(self, df, timeframe_label: str) -> tuple[str, float]:
        ...
        try:
            ...
            logger.debug(
                f"🔍 ADX DIAGNOSTIC [{timeframe_label}]: +DI={plus_di_final:.1f if plus_di_final is not None else None}, -DI={minus_di_final:.1f if minus_di_final is not None else None}"
            )
        ...
        try:
            ...
            if trend == "bullish":
                if has_strong_momentum and normalized_slope > strong_slope_threshold:
                    ...
                    logger.info(
                        f"Regime {timeframe_label}: STRONG_UP (swing=bullish, slope={normalized_slope:.1f}, ADX={adx_str})"
                    )
                ...
                if has_strong_momentum and normalized_slope < -strong_slope_threshold:
                    adx_str = f"{adx_value:.1f}" if adx_value is not None else "N/A"
                    logger.info(
                        f"Regime {timeframe_label}: STRONG_DOWN (swing=bearish, slope={normalized_slope:.1f}, ADX={adx_str})"
                    )
    ...
    def _apply_hysteresis(self, new_regime: MarketRegime) -> MarketRegime:
        ...
        if all(label == last_regime.composite for label in recent_labels):
            ...
            if len(self.regime_history) > 20:
                self.regime_history = self.regime_history[-20:]
            logger.debug(
                "Regime hysteresis: keeping %s despite new %s",
                last_regime.composite,
