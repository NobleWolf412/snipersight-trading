...
def is_price_extended(
    df: pd.DataFrame, threshold_pct: float = 3.0, ema_period: int = 20
...
def is_volume_exhausted(
    df: pd.DataFrame, lookback: int = 4, spike_threshold: float = 2.0
) -> tuple[bool, str]:
    """
    ...
    if len(candle_colors) >= 3:
        ...
        if all(c == "green" for c in last_3_colors):
            declining = all(
                last_3_volumes[i] > last_3_volumes[i + 1] for i in range(len(last_3_volumes) - 1)
            )
            if declining:
    ...
    if last_is_green != prev_is_green:
        if last_candle["volume"] >= avg_volume * spike_threshold:
            ...
            return (
                True,
                f"Volume spike on {color} reversal candle ({last_candle['volume']/avg_volume:.1f}x avg)",
            )
...
def check_smc_pullback_confirmation(smc_snapshot, pullback_direction: str) -> tuple[bool, str]:
    """
