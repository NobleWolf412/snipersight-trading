...
class DominanceContext:
    ...
    def get_series(self) -> Tuple[List[Tuple[float, float]], List[Tuple[float, float]], List[Tuple[float, float]]]:
        """
    ...
    def get_dominance(self, force_refresh: bool = False) -> Optional[DominanceSnapshot]:
        ...
        if not force_refresh:
            ...
            if cache and self._is_cache_valid(cache):
                logger.debug("Using cached dominance data (age: %.0f min)", 
                ...
                with open(history_file, 'r') as f:
                    ...
                    for entry in history_data:
                        if entry.get('timestamp', 0) >= cutoff:
                            history.append(DominanceSnapshot(
