...
class NotificationManager:
    ...
    def send_risk_alert(
        self, alert_type: str, message: str, severity: str = "warning", data: Optional[Dict] = None
    ) -> str:
        """Send risk management alert."""
        event_id = f"risk-{alert_type}-{int(datetime.now().timestamp())}"

        priority = (
            NotificationPriority.CRITICAL if severity == "critical" else NotificationPriority.HIGH
        )
    ...
    def send_execution_notification(self, execution_data: Dict[str, Any]) -> str:
        ...
        status_emoji = {"filled": "✅", "partial": "🟡", "cancelled": "❌", "rejected": "🚫"}
        ...
        event = NotificationEvent(
            ...
            priority=(
                NotificationPriority.HIGH if status == "rejected" else NotificationPriority.NORMAL
            ),
            timestamp=datetime.now(timezone.utc),
    ...
    def send_system_notification(
        self, title: str, body: str, priority: str = "normal", action: Optional[str] = None
    ) -> str:
